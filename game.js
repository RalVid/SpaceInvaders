...
const explosionDuration = 20, popupDuration = 60;
...
function playGameOverMelody() {
  ...
  const now = audioCtx.currentTime, duration = 1.5;
...
function resizeCanvas() {
  ...
  // clamp between “never shrink below fit” and max 1.4× upscale
...
function unlockAudio(){
  if (audioCtx.state === "suspended") audioCtx.resume();
  ["click","pointerdown","keydown"].forEach(e => window.removeEventListener(e, unlockAudio));
}
["click","pointerdown","keydown"].forEach(e => window.addEventListener(e, unlockAudio));
...
// ==== WAIT FOR SPRITES LOAD & SHOW MENU ====
let loaded = 0;
...
function startGame(){
  ...
  player = { x:225, y:550, width:50, height:20, speed:5, bullets:[], cooldown:0, blink:0 };
  enemies      = [];
  enemyBullets = [];
  shields      = [];
  explosions   = [];
...
function update(dt){
  ...
  if (!mysteryShip){
    ...
    mysteryShip.x += mysteryShip.speed * dt * 60;
    if (ufoOsc){
  ...
  enemies.forEach(e=>{
    if (!e.alive) return;
    e.x += enemyDirection*enemySpeed*dt*60;
  ...
  if (keys["ArrowRight"]&&player.x+player.width<GAME_W) player.x+=player.speed*dt*60;
  if (keys[" "]) shoot();
  if (player.cooldown>0) player.cooldown--;

  // bullets
  player.bullets.forEach((b,i)=>{ b.y-=b.speed*dt*60; if(b.y<0) player.bullets.splice(i,1); });
  enemyBullets.forEach((b,i)=>{ b.y+=b.speed*dt*60; if(b.y>GAME_H) enemyBullets.splice(i,1); });
  ...
  enemies.forEach(e=>{
    if (e.alive&&e.y+e.height>=player.y){
      e.alive=false; handlePlayerHit(e.x+e.width/2,e.y+e.height/2);
    }
  });
...
function handleCollisions(){
  ...
  player.bullets=player.bullets.filter(b=>{
    if (mysteryShip&&
        b.x<mysteryShip.x+mysteryShip.width&&b.x+b.width>mysteryShip.x&&
      ...
      playUfoHitSound(); addPopup(b.x,b.y,`+${bonus}`); addExplosion(b.x,b.y);
      mysteryShip=null; mysteryTimer=getRandomMysteryFrames();
      if(ufoOsc){ufoOsc.stop();ufoOsc=null;ufoGain=null;}
      return false;
    }
  ...
  enemyBullets=enemyBullets.filter(b=>{
    if(b.x<player.x+player.width&&b.x+b.width>player.x&&b.y<player.y+player.height&&b.y+b.height>player.y){
      handlePlayerHit(player.x+player.width/2,player.y); return false;
    }
    return true;
  });
    ...
    return list.filter(b=>{
      for(let i=0;i<arr.length;i++){
        ...
        if(b.x<s.x+s.width&&b.x+b.width>s.x&&b.y<s.y+s.height&&b.y+b.height>s.y){
          arr.splice(i,1); addExplosion(b.x,b.y); return false;
        }
      }
      return true;
    });
...
function draw(){
  ...
  // HUD
  ctx.fillStyle="red";
  ctx.font="21px monospace";  // larger font
  ctx.textBaseline="top";

  const pad4 = str=>str.toString().padStart(4,"0");

  ctx.fillText("SCORE:", 50, 10);
  ctx.fillText(pad4(score), 50 + ctx.measureText("SCORE: ").width, 10);

  // const hiText=`HI-SCORE: ${pad4(highscore)}`;
  //const hiX=GAME_W/2 - ctx.measureText(hiText).width/2;
  //ctx.fillText(hiText, hiX, 10);

  const livesText=`LIVES: ${lives}`;
  ctx.fillText(livesText,
               GAME_W - ctx.measureText(livesText).width,
               10);

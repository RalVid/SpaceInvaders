...
for (let r = 0; r < FRAMES; r++) {
  for (let c = 0; c < TYPES; c++) {
    ENEMY_FRAMES.push({
      sx: c*FRAME_W, sy: r*FRAME_H, w: FRAME_W, h: FRAME_H
    });
  }
}
let enemyFrame = 0;
...
// ==== TOUCH CONTROL ZONES ====
// Lower zone (ty >= player.y): steer ship left/right by touching left/right of its center
...
function update() {
  ...
  explosions.forEach((ex,i)=>{
    if (++ex.frame > explosionDuration) explosions.splice(i,1);
  });

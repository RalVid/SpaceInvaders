...
const popupDuration     = 60;  // frames for UFO point popups

// UFO state
let mysteryShip, mysteryTimer, ufoOsc, ufoGain;

// drop cooldown to prevent repeated boundary-drops
let dropCooldown = 0;

// logical canvas size
const GAME_W = 500, GAME_H = 600;

// timing for variable timestep
let lastTime = 0;
...
function addPopup(x,y,text){
  popups.push({ x,y,text,frame:0 });
}
function updatePopups(){
  popups = popups.filter(p => ++p.frame <= popupDuration);
}
function drawPopups(){
...
function handlePlayerHit(x,y){
  ...
  player.blink = 30;  // frames to blink
  if (lives === 0){
    gameOver = true;
...
function draw(){
  ...
  const hiText=`HI-SCORE: ${highscore.toString().padStart(4,"0")}`,
        hiX = GAME_W/2 - ctx.measureText(hiText).width/2;

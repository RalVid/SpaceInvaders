...
function bindControlButton(btn, key) {
...
bindControlButton(leftBtn,  "ArrowLeft");
bindControlButton(rightBtn, "ArrowRight");
bindControlButton(shootBtn, " ");
...
function update(){
  ...
  if (Math.random() < 0.02){
    ...
    if (alive.length) enemyShoot(alive[Math.floor(Math.random()*alive.length)]);
  }
